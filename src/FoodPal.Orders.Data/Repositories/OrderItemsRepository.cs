...
namespace FoodPal.Orders.Data.Repositories
{
    public class OrderItemsRepository : IOrderItemsRepository
    {
        ...
        public async Task<List<OrderItem>> GetItemsAsync(int orderId)
        {
            ...
            {
                throw new Exception($"Orders item by orderId: {orderId} could not be found. Reason:{ex.Message}");
            }
        }
        ...
        public async Task UpdateStatusAsync(OrderItem orderItemEntity, OrderItemStatus newStatus)
        {
            try
            {
                var orderItem = await _ordersContext.OrderItems.SingleOrDefaultAsync(oi => oi.Id == orderItemEntity.Id);
                if (orderItem != null)
                {
