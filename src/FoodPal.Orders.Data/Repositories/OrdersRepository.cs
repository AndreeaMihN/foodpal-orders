...
namespace FoodPal.Orders.Data.Repositories
{
    public class OrdersRepository : IOrdersRepository
    {
        ...
        public async Task<Order> GetByIdAsync(int orderId)
        {
            try
            {
                return await _ordersContext.Orders.SingleOrDefaultAsync(o => o.Id == orderId);
            }
            catch (Exception ex)
            {
                throw new Exception($"Order by {orderId} could not be found. Reason:{ex.Message}");
            }
        }
        ...
        public async Task<OrderStatus?> GetStatusAsync(int orderId)
        {
            try
            {
                //var order = await _ordersContext.Orders.SingleOrDefaultAsync(o => o.Id == orderId);
                //return order.Status;
